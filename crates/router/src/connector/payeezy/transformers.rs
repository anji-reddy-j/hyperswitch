use crate::{
    core::errors,
    types::{self, api, storage::enums}, utils::OptionExt,
};
...
impl TryFrom<&types::PaymentsAuthorizeRouterData> for PayeezyPaymentsRequest {
    ...
    fn try_from(_item: &types::PaymentsAuthorizeRouterData) -> Result<Self, Self::Error> {
        ...
        let credit_card = PayeezyCreditCard {
            ...
            card_number: "4788250000028291".to_string(),
...
pub struct PayeezyPaymentsResponse {
    ...
    pub validation_status: PayeezyPaymentStatus,
    ...
    for types::RouterData<F, T, types::PaymentsResponseData>
{
    ...
    fn try_from(
        ...
        Ok(Self {
            status: enums::AttemptStatus::from(item.response.validation_status),
            response: Ok(types::PaymentsResponseData::TransactionResponse {
                resource_id: types::ResponseId::ConnectorTransactionId(item.response.transaction_id),
                redirection_data: None,
                redirect: false,
                mandate_reference: None,
                connector_metadata: Some(serde_json::to_value(PayeezyMeta{ tag: item.response.transaction_tag.to_string()}).into_report().change_context(errors::ParsingError)?, ),
            }),
            ..item.data
        })
    }
        ...
        fn try_from(
            ...
            Ok(Self {
                ...
                response: Ok(types::PaymentsResponseData::TransactionResponse {
                    ...
                    redirection_data: None,
                    mandate_reference: None,
                    connector_metadata: Some(serde_json::to_value(PayeezyMeta{ tag: item.response.transaction_tag.to_string()}).into_report().change_context(errors::ConnectorError::MissingRequiredField { field_name: ("metadata") })?, ),
                }),
